...
def analyze_text(request):
    ...
    result = _save_result(
        raw_text, madhab, report, product_name=request.POST.get("product_name", "")
    )
...
def analyze_barcode(request):
    ...
    if not product["found"]:
        messages.error(
            request, f"Barcode not found: {product.get('error', 'Unknown error')}"
        )
        return render(request, "core/index.html", {"madhab_choices": Madhab.choices})
    ...
    if not ingredients_text:
        messages.warning(
            request,
            f"Product '{product['product_name']}' found but has no ingredient list.",
        )
        return render(request, "core/index.html", {"madhab_choices": Madhab.choices})
...
def analyze_image(request):
    ...
    if not ingredients_text:
        messages.error(
            request,
            "Could not find an ingredient list in the image. Try a clearer photo.",
        )
        return render(request, "core/index.html", {"madhab_choices": Madhab.choices})
    ...
    if has_halal_logo:
        ...
        overall = (
            max(results, key=lambda r: STATUS_PRIORITY.get(r.status, 0)).status
            if results
            else IngredientStatus.QUESTIONABLE
        )
